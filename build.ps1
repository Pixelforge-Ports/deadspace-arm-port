...
try {
    $buildArguments = @('build', '--progress=plain', '-t', 'deadspace-build', '-f', 'Dockerfile.build')
    ...
    & $dockerCommand.Source run --rm --mount "type=bind,source=$PSScriptRoot,target=/src" -w /src deadspace-build bash -lc 'make -j2 && make libs && bash package_portmaster.sh'
    if ($LASTEXITCODE -ne 0) { throw 'Port compilation or packaging failed; see the error above.' }
    Write-Host "Built: $PSScriptRoot/build/deadspace-portmaster.zip"
