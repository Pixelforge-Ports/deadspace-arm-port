...
export DEADSPACE_RESOLUTION="${DEADSPACE_RESOLUTION:-auto}"
if [ -f "$GAMEDIR/resolution.txt" ]; then
  export DEADSPACE_RESOLUTION="$(tr -d '\r\n' < "$GAMEDIR/resolution.txt")"
fi
...
export DEADSPACE_FACE_LAYOUT="${DEADSPACE_FACE_LAYOUT:-$_face_default}"
...
EOF
  ...
  if command -v pm_message >/dev/null 2>&1; then
    pm_message "Dead Space: preparing game data. Keep the device powered on."
...
gl_mesa_vendor_loads() {
  ...
  LD_LIBRARY_PATH="$GL_SHIM:$LD_LIBRARY_PATH" \
    "$GAMEDIR/deadspace" --gl-probe "$GL_MESA_DIR/libEGL_mesa.so.0" >/dev/null 2>&1
}
...
gl_mesa_unshadow() {
  ...
  "$GAMEDIR/deadspace" --gl-probe-deps "$GL_MESA_DIR/libEGL_mesa.so.0" 2>&1 | sed 's/^/GL:   /'
...
if [ -n "$MALI_BLOB" ]; then
  ...
  if [ -n "$GL_EGL" ]; then
	...
	# Mesa/Panfrost owns the current GL context. Force the
	# fixed-function GLES1 table to use the same driver's
	# SDL_GL_GetProcAddress() dispatch instead of independently
	# opening libmali.so.1.
	export DEADSPACE_GL_SINGLE_DISPATCH=1
	echo "GL: Mesa/Panfrost selected; forcing GLES1 through SDL single-driver dispatch"
...
mapper_pid=
if [ -n "${GPTOKEYB2:-}" ]; then
  $GPTOKEYB2 "deadspace" -c "$GAMEDIR/deadspace.ini" &
  mapper_pid=$!
fi
